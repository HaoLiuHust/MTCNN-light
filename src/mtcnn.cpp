...
Pnet::Pnet() {
	...
	string filename = "../model/mtcnn/Pnet.txt";
...
void Pnet::run(Mat &image, float scale) {
	...
	image2Matrix(image, this->rgb);//?????change image to rgb?
...
Rnet::Rnet() {
	...
	string filename = "../model/mtcnn/Rnet.txt";
	readData(filename, dataNumber, pointTeam);
...
Onet::Onet() {
	...
	string filename = "../model/mtcnn/Onet.txt";
	readData(filename, dataNumber, pointTeam);
...
void mtcnn::findFace(Mat &image) {
	...
	for (vector<struct Bbox>::iterator it = firstBbox_.begin(); it != firstBbox_.end(); it++) {
		if ((*it).exist) {
			...
			Mat secImage;
			if (temp.x<0 || temp.y<0 || temp.width <= 0 || temp.height <= 0 || image.empty() || image(temp).empty())
				continue;
...
int mtcnn::detectFace(const Mat &image, vector<struct Bbox>& facebox) {
	facebox.clear();
	struct orderScore order;
	int count = 0;
	for (size_t i = 0; i < scales_.size(); i++) {
		...
		if (image.empty())
			continue;
		resize(image, reImage, Size(changedW, changedH), 0, 0, cv::INTER_LINEAR);
		simpleFace_[i].run(reImage, scales_.at(i));//p-net
		nms(simpleFace_[i].boundingBox_, simpleFace_[i].bboxScore_, simpleFace_[i].nms_threshold);
	...
	int index = 0;
	for (vector<struct Bbox>::iterator it = thirdBbox_.begin(); it != thirdBbox_.end(); it++, ++index) {
		if (it->exist) {
